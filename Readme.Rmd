...
```{r datacleanse, include=FALSE}
df<-read.csv("C:\\Users\\YAshroff\\Documents\\personal\\scotia\\1SP500.csv",header=TRUE,na.strings=c("."))
#remove null rows
df<-na.omit(df)
...
#break histogram into 100 buckets 
hist(df$VALUE,breaks=200, xaxt="n")
...
```{r pressure, echo=FALSE}
 ggplot(df, aes(x = df$VALUE)) + geom_histogram(aes(y = ..density..), binwidth = 0.1) + geom_density() + labs(x = "Occurences of each average closing index")
